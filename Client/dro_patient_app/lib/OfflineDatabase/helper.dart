...
class DatabaseHelper{
  ...
  void _onCreate(Database db, int version) async {
    ...
    await db.execute("CREATE TABLE $tableName($cId INTEGER PRIMARY KEY, $cEmail TEXT,$cFName TEXT, $cLName TEXT ,$cStatus INTEGER);");
  ...
  Future<int> saveUser(OfflinePatient user) async{
    var dbClient = await db;
    int res = await dbClient.insert('$tableName', user.toMap());
    return res;
  }
  Future<OfflinePatient> getUser() async{
    var dbClient = await db;

    var res = await dbClient.rawQuery("SELECT * FROM $tableName");

    if(res.length == 0){
      return OfflinePatient(status: '0');
    }else{
      return OfflinePatient.fromMap(res.first);
    }
  }
  ...
  Future updateName(OfflinePatient user) async{
    var dbClient = await db;
    await dbClient.update(tableName, user.toMap(),where: '$cId = ?',whereArgs: [user.id]);
  }
